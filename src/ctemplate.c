...
void write_file(char* path)
{
    ...
    for (int i = 0; i < template_file_line_count; i++) {
        ...
        if (strncmp(template_file[i], "%TEMPLATE_BEGIN%", strlen("%TEMPLATE_BEGIN%")) == 0) {
            ...
            for (int k = 0; k < types_count; k++) {
                ...
                for (int j = 0; j < type_strlen; j++) {
                    if (types[k][j] == '*') type_ptr_count++;
                }

                int j = i + 1;
                for (; strncmp(template_file[j], "%TEMPLATE_END%", strlen("%TEMPLATE_END%")) != 0 && j < template_file_line_count; j++) {
