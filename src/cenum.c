...
void read_enum_file(char* path)
{
    ...
    while ((linelen = getline(&line, &linecap, f)) > 0) {
        ...
        if (strncmp("ENUM", line, strlen("ENUM")) == 0) {
            memcpy(decl[decl_len++], (char*)(line + i + 1), linelen - i - 2);
            cur_value_index = 0;
        }
...
void write_implementation(char* dir)
{
    ...
    char decl_uppercase[MAX_STRLEN];
    char decl_full_name[MAX_STRLEN];
    for (int i = 0; i < decl_len; i++) {
        int decl_str_len = strlen(decl[i]);
        memcpy(decl_uppercase, decl[i], decl_str_len);
        decl_uppercase[decl_str_len] = '\0';
        for (int j = 0; j < decl_str_len; j++)
            decl_uppercase[j] = toupper(decl_uppercase[j]);

        fprintf(f, "const char* toString_%s(%s value)\n", decl[i], decl[i]);
        fprintf(f, "{\n");
        fprintf(f, "    switch (value) {\n");
        for (int j = 0; j < decl_values_lens[i]; j++) {
            sprintf(decl_full_name, "%s_%s", decl_uppercase, decl_values[i][j]);
